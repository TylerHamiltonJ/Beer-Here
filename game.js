...
function controlGame() {
  switch (state.current) {
    case state.getReady:
      errorMessage.x = randomIntFromInterval(0, 3) * 130;
      errorMessage.y = randomIntFromInterval(0, 1) * 98 + 469;
...
const gameOver = {
  ...
  draw: function () {
    ...
    if (state.current == state.over) {
      ...
      ctx.fillStyle = "#222";
      ctx.font = "18px VT323";
      console.log(errorMessage.x, errorMessage.y);
      ctx.drawImage(
        sprite,
        errorMessage.x,
        errorMessage.y,
